...
public class ProveedorRequestDto {
	...
	@Pattern(regexp = "^\\d{10}(\\d{3})?$",
			message = "Debe ingresar 10 dígitos para cédula o 13 para RUC")
	...
	public boolean isIdentificacionEcuatorianaValida() {
		if (identificacion == null || !identificacion.matches("\\d{10}|\\d{13}")) return true;
		return identificacion.length() == 10 ? validarCedula(identificacion) : validarRuc(identificacion);
	}

	private boolean validarRuc(String valor) {
		int tercerDigito = Character.getNumericValue(valor.charAt(2));
		if (tercerDigito < 6) return validarCedula(valor.substring(0, 10)) && valor.endsWith("001");
		if (tercerDigito == 6)
	...
	private boolean validarCedula(String valor) {
		...
		if (provincia < 1 || provincia > 24 || Character.getNumericValue(valor.charAt(2)) >= 6) return false;
		int suma = 0;
		for (int i = 0; i < 9; i++) {
